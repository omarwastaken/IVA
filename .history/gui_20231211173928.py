...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        self.refresh_chat_view(bubble_id)
        return bubble_id
    ...
    def refresh_chat_view(self, bubble_id=None):
        """Refresh the view of the chat canvas, optionally scrolling to a new message bubble."""
        self.chat_canvas.configure(scrollregion=self.chat_canvas.bbox("all"))
        
        # If a bubble_id is provided, scroll to it
        if bubble_id is not None:
            bubble, message_label_window = self.chat_bubbles[bubble_id]
            self.chat_canvas.yview('moveto', self.chat_canvas.coords(message_label_window)[1] / self.chat_canvas.bbox("all")[3])
        else:
