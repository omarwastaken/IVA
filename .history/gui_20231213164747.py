...
class ChatInterface:
    ...
    def toggle_input_state(self):
        ...
        if self.mic_mode:  # If currently in microphone mode, switch to text mode
            self.text_field['state'] = tk.NORMAL
            self.send_button['state'] = tk.NORMAL
            self.mic_mode = False
        else:  # If currently in text mode, switch to microphone mode
    ...
    def listen_and_convert_speech(self):
        """Listen to speech and convert it to text in a loop."""
        while self.mic_mode:
