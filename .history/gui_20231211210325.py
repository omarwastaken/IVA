...
class ChatInterface:
    ...
    def __init__(self, root):
        ...
        self.next_bubble_y_position = self.START_MARGIN
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        bubble = self.create_rounded_rectangle(bubble_x1, self.next_bubble_y_position, bubble_x2, self.next_bubble_y_position + label_height, radius=self.BUBBLE_RADIUS, fill=self.BUBBLE_COLOR)
        message_label_window = self.create_message_label(message, self.next_bubble_y_position, label_height, window_x)
        self.chat_bubbles.append((bubble, message_label_window))
        
        # Update the position for the next bubble
        self.next_bubble_y_position += label_height + self.BUBBLE_SPACING
