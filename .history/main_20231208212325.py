...
def talk_iva():
    ...
    while True:
        if keyboard.is_pressed('ctrl+m'):
            ...
            speak(f"Switched to {input_mode} mode")
            continue  # Skip the rest of the loop to let the mode switch take effect

        user_input = get_user_input(input_mode)
