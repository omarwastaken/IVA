...
class ChatInterface:
    ...
    def setup_reminders_components(self):
        ...
        self.start_periodic_reminder_update()

    def start_periodic_reminder_update(self):
        """Start the periodic reminders update in a separate thread."""
        self.update_thread = threading.Thread(target=self.update_reminders_periodically, daemon=True)
        self.update_thread.start()

    def update_reminders_periodically(self):
        """Periodically fetch and update reminders in the Todo list box."""
        while True:  # Infinite loop to keep updating the reminders
            try:
                self.display_reminders()
            except Exception as e:
