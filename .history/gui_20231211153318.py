...
class ChatInterface:
    ...
    def create_message_label(self, message, y_position, label_height, window_x, is_typing=False):
        message_label = tk.Label(self.chat_canvas, text=message, bg=self.BUBBLE_COLOR, fg="black",
                                 font=self.customFont, wraplength=self.TEXT_WIDTH_LIMIT - 2 * self.PADDING, justify='left')
        if is_typing:
            # Place the typing label at a fixed position for now
            typing_y_position = self.calculate_y_position() + 40  # Adjust as needed
            message_label_window = self.chat_canvas.create_window(self.CHAT_AREA_WIDTH // 2, typing_y_position, window=message_label)
            return message_label, message_label_window
        else:
    ...
    def show_typing_animation(self):
        ...
        if self.typing_animation_id is None:
            ...
            self.typing_label, self.typing_bubble_id = self.create_message_label(self.typing_message, 0, 0, 0, is_typing=True)
            self.animate_typing()

    def animate_typing(self):
        """Animate the typing message with dots."""
        if self.typing_animation_id is not None and self.typing_label is not None:
            dots = self.typing_message.count('.') % 3 + 1
            self.typing_message = "IVA is typing" + "." * dots
            self.typing_label.config(text=self.typing_message)
