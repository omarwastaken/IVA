...
class ChatInterface:
    ...
    def show_typing_animation(self):
        ...
        if not self.typing_animation_id:
            self.typing_message = "IVA is typing"
            self.typing_label = self.add_typing_label(self.typing_message)
            self.animate_typing()

    def add_typing_label(self, message):
        """Create and display a typing label."""
        y_position = self.calculate_y_position() + self.BUBBLE_SPACING
        label = tk.Label(self.chat_canvas, text=message, bg=self.BUBBLE_COLOR, fg="black", font=self.customFont)
        self.chat_canvas.create_window(self.CHAT_AREA_WIDTH // 2, y_position, window=label, anchor="n")
        return label
