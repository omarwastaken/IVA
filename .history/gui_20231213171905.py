...
class ChatInterface:
    ...
    def toggle_input_state(self):
        """Toggle the input state between voice and text."""
        self.mic_mode = not self.mic_mode
        if self.mic_mode:  # If switched to microphone mode
            self.text_field['state'] = tk.DISABLED
            self.send_button['state'] = tk.DISABLED
            self.listen_and_convert_speech()  # Start listening immediately
        else:  # If switched back to text mode
            self.text_field['state'] = tk.NORMAL
            self.send_button['state'] = tk.NORMAL

    def listen_and_convert_speech(self):
        """Listen to speech and convert it to text, then start listening again after AI response."""
        if self.mic_mode:  # Check if microphone mode is active
            text = recognize_speech()  # From utils.py
            if text:
                self.text_field['state'] = tk.NORMAL
                self.text_field.delete(0, tk.END)
                self.text_field.insert(0, text)
                self.send_message()  # Automatically send the converted message
                self.text_field['state'] = tk.DISABLED
                self.root.after(1000, self.listen_and_convert_speech)  # Wait a bit and start listening again             
