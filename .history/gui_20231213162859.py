...
class ChatInterface:
    ...
    def __init__(self, root):
        ...
        try:
            self.setup_ui()
            self.initial_ai_message()  # Call a new method to handle the initial AI message
    ...
    def toggle_input_state(self):
        ...
        if self.text_field['state'] == tk.NORMAL:
            self.text_field['state'] = tk.DISABLED
            self.send_button['state'] = tk.DISABLED
            self.handle_voice_input()  # Call a method to handle voice input
        else:
            self.text_field['state'] = tk.NORMAL
            self.send_button['state'] = tk.NORMAL   

    def handle_voice_input(self):
        """Start a new thread for handling voice input."""
        self.voice_input_thread = threading.Thread(target=self.voice_input_loop, daemon=True)
        self.voice_input_thread.start()

    def voice_input_loop(self):
        """Continuously handle voice input when the microphone button is pressed."""
        while self.text_field['state'] == tk.DISABLED:
            voice_input = recognize_speech()
            if voice_input:
                self.root.after(0, lambda: self.add_message_bubble(voice_input, is_user=True))
                threading.Thread(target=self.process_and_display_response, args=(voice_input,), daemon=True).start()               
