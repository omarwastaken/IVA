...
class ChatInterface:
    ...
    def setup_feature_components(self):
        ...
        self.todo_list_text = tk.Text(self.features_frame, bg="white", fg="black", font=font.Font(family="Helvetica", size=12))
        self.todo_list_text.pack(padx=20, pady=10, fill=tk.BOTH, expand=True)
        scrollbar = tk.Scrollbar(self.features_frame, command=self.todo_list_text.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.todo_list_text.configure(yscrollcommand=scrollbar.set)
        self.display_todays_reminders()

    def on_checkbox_clicked(event_id):
        """Callback function for when a checkbox is clicked."""
        mark_reminder_as_done(event_id)
        ...
        try:
            ...
            if reminders:
                for reminder in reminders:
                    datetime_str = reminder['start']['dateTime']
                    datetime_obj = datetime.fromisoformat(datetime_str)
                    local_timezone = pytz.timezone(reminder['start']['timeZone'])
                    local_datetime = datetime_obj.astimezone(local_timezone)
                    formatted_datetime = local_datetime.strftime('%H:%M, %A, %d %B %Y')
                    self.todo_list_text.insert(tk.END, reminder['summary'] + '\n', 'bold')
                    self.todo_list_text.insert(tk.END, formatted_datetime + '\n', 'regular')
                    self.todo_list_text.insert(tk.END, "-" * 50 + '\n', 'separator')
                    var = tk.BooleanVar()
                    chk = ttk.Checkbutton(self.todo_list_text, text='', variable=var)
                    chk.pack()
                    chk.bind('<Button-1>', lambda event, eid=reminder['id']: self.on_checkbox_clicked(eid))
            else:
                self.todo_list_text.insert(tk.END, "No reminders for today.\n", 'regular')
            self.todo_list_text.tag_configure('bold', font=font.Font(family="Helvetica", size=12, weight="bold"))
            self.todo_list_text.tag_configure('regular', font=font.Font(family="Helvetica", size=12))
            self.todo_list_text.tag_configure('separator', font=font.Font(family="Helvetica", size=12), foreground='grey')
            self.todo_list_text.configure(state='disabled')
        except Exception as e:
            self.todo_list_text.insert(tk.END, f"Error fetching reminders: {e}\n", 'regular')
            self.todo_list_text.configure(state='disabled')
