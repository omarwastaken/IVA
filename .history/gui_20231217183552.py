...
from main import process_input, handle_reminder_request
from modules.utils import speak, recognize_speech
from modules.reminders import is_reminder_request, get_reminders_for_period, mark_reminder_as_done
...
class ChatInterface:
 ...
 # ======================== Chat Feature ========================
    ...
    def process_text_input(self, message):
        ...
        if is_reminder_request(message):  # Check if it's a reminder request
            reminder_responses = handle_reminder_request(message)
            for response in reminder_responses:
                self.append_message("➢ IVA: " + response)
        else:
    ...
    def speech_recognition_and_response(self):
        ...
        if text:
            ...
            self.root.after(0, self.start_typing_animation)
            if is_reminder_request(text):  # Check if it's a reminder request
                reminder_responses = handle_reminder_request(text)
                for response in reminder_responses:
                    self.root.after(0, lambda: self.append_message("➢ IVA: " + response))
            else:
