...
class ChatInterface:
    ...
    def setup_main_window(self):
        ...
        # Gradient background
        self.gradient_canvas = tk.Canvas(self.root)
    ...
    def on_window_resize(self, event):
        ...
        if self.last_width != event.width or self.last_height != event.height:
            self.last_width, self.last_height = event.width, event.height
            self.update_gradient(event.width, event.height)

    def update_gradient(self, width, height):
        # Disconnect the event to prevent a loop
        self.root.unbind("<Configure>")
        
        # Update the canvas and redraw the gradient
        self.gradient_canvas.config(width=width, height=height)
        self.gradient_canvas.delete("all")  # Clear existing gradient
        self.gradient = self.create_gradient(self.gradient_canvas, width, height, "#f0f0f0", "#202225")
