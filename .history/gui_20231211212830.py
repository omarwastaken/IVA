...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        if not self.chat_bubbles:
            # Ensure the first bubble starts at START_MARGIN from the top.
            y_position = self.START_MARGIN
        else:
            last_bubble = self.chat_bubbles[-1]
            last_bubble_coords = self.chat_canvas.coords(last_bubble[0])
            # Ensure subsequent bubbles are spaced with BUBBLE_SPACING from the previous one.
            y_position = last_bubble_coords[3] + self.BUBBLE_SPACING

        # Create the bubble and the message label inside it.
        ...
        # Refresh the chat view to update the scroll area and ensure the latest message is visible.
    ...
    def calculate_bubble_position(self, label_width, is_user):
        """Calculate bubble starting and ending positions and the message label window position."""
        if is_user:
            bubble_x1 = self.CHAT_AREA_WIDTH - label_width - self.LABEL_OFFSET - self.PADDING
            bubble_x2 = self.CHAT_AREA_WIDTH - self.LABEL_OFFSET - self.PADDING
            window_x = self.CHAT_AREA_WIDTH - label_width - self.LABEL_OFFSET - (self.PADDING * 2)
        else:
            bubble_x1 = self.LABEL_OFFSET + self.PADDING
            bubble_x2 = label_width + self.LABEL_OFFSET + self.PADDING
            window_x = self.LABEL_OFFSET + self.PADDING * 2
    ...
    def create_message_label(self, message, y_position, label_height, window_x):
        """Create the message label and place it inside the bubble with appropriate padding."""
        message_label = tk.Label(self.chat_canvas, text=message, bg=self.BUBBLE_COLOR, fg="black",
                                 font=self.customFont, wraplength=self.TEXT_WIDTH_LIMIT - self.PADDING * 2, justify='left')
        message_label_window = self.chat_canvas.create_window(window_x, y_position + self.PADDING, window=message_label, anchor="nw")
