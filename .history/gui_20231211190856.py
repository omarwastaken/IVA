...
class ChatInterface:
    ...
    def refresh_chat_view(self):
        """Refresh the view of the chat canvas and reposition message bubbles."""
        self.reposition_bubbles()
        self.chat_canvas.configure(scrollregion=self.chat_canvas.bbox("all"))
        self.chat_canvas.yview_moveto(1)

    def reposition_bubbles(self):
        """Reposition all the message bubbles."""
        y_position = self.START_MARGIN
        for bubble_id, (bubble, message_label_window) in enumerate(self.chat_bubbles):
            _, label_height = self.calculate_label_dimensions(self.get_message_label_text(message_label_window))
            self.chat_canvas.coords(bubble, self.chat_canvas.coords(bubble)[:2] + [self.chat_canvas.coords(bubble)[2], y_position + label_height])
            self.chat_canvas.coords(message_label_window, self.chat_canvas.coords(message_label_window)[0], y_position + label_height / 2)
            y_position += label_height + self.BUBBLE_SPACING

    def get_message_label_text(self, message_label_window):
        """Get the text of a message label."""
        if str(message_label_window) in self.chat_canvas.children:
            message_label = self.chat_canvas.nametowidget(message_label_window)
            return message_label.cget("text")
        return ""                
