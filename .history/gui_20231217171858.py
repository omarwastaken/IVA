...
class ChatInterface:
 ...
 # ======================== Chat Feature ========================
    ...
    """Process text input based on current application state."""
    global reminder_creation_state
    # Start typing animation
    self.root.after(0, self.start_typing_animation)

    if reminder_creation_state:
        # Process reminder-related input
        process_reminder_input(message, self.update_gui_with_message)
    else:
        # Normal processing
...
def update_gui_with_message(self, message):
    """Updates the GUI with a given message."""
    self.root.after(0, lambda: self.append_message("➢ IVA: " + message))
