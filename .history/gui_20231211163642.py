...
class ChatInterface:
    ...
    def create_rounded_rectangle_coords(self, x1, label_height, x2):
        """Create coordinates for a rounded rectangle."""
        y1 = self.chat_canvas.coords(self.chat_bubbles[self.typing_bubble_id][0])[1]
        y2 = y1 + label_height
        return [x1+self.BUBBLE_RADIUS, y1, x1+self.BUBBLE_RADIUS, y1, x2-self.BUBBLE_RADIUS, y1, 
                x2-self.BUBBLE_RADIUS, y1, x2, y1, x2, y1+self.BUBBLE_RADIUS, x2, y2-self.BUBBLE_RADIUS,
                x2, y2-self.BUBBLE_RADIUS, x2, y2, x2-self.BUBBLE_RADIUS, y2, x1+self.BUBBLE_RADIUS, y2,
                x1+self.BUBBLE_RADIUS, y2, x1, y2, x1, y2-self.BUBBLE_RADIUS, x1, y1+self.BUBBLE_RADIUS,
                x1, y1+self.BUBBLE_RADIUS, x1, y1]
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        return self.create_and_add_bubble(bubble_x1, y_position, bubble_x2, label_height, message, window_x)
    ...
    def update_message_bubble(self, bubble_id, new_message):
        ...
        if bubble_id is not None and 0 <= bubble_id < len(self.chat_bubbles):
            bubble, message_label_window = self.chat_bubbles[bubble_id]
            ...
            label_width, label_height = self.calculate_label_dimensions(new_message)
            bubble_x1, bubble_x2, _ = self.calculate_bubble_position(label_width, is_user=False)
            self.chat_canvas.coords(bubble, self.create_rounded_rectangle_coords(bubble_x1, label_height, bubble_x2))

    def hide_typing_animation(self):
        """Hide the typing animation from the chat interface."""
        # If typing animation is active, stop it before trying to delete the bubble
        if self.is_typing_animation_active and self.typing_animation_id is not None:
            self.root.after_cancel(self.typing_animation_id)
            self.typing_animation_id = None
            self.is_typing_animation_active = False
        
        # No need to delete the bubble if we're going to update it with AI's response
        # Just reset the animation variables
