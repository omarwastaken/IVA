...
class ChatInterface:
    ...
    def refresh_reminders(self):
        """Refresh the reminders every minute."""
        while True:
            self.display_reminders()
            time.sleep(60)

    def start_refresh_thread(self):
        """Start the thread that will refresh reminders."""
        refresh_thread = threading.Thread(target=self.refresh_reminders, daemon=True)
    ...
    def display_reminders(self):
        ...
        try:
            ...
            if self.root.winfo_exists():  # Check if the root window exists
                self.root.after(0, self.update_reminders_treeview, reminders)
        except Exception as e:
            if self.root.winfo_exists():
                self.root.after(0, lambda: self.todo_list_tree.insert('', tk.END, text='', values=(f"Error fetching reminders: {e}", "")))

    # New helper method to update the Treeview widget
    def update_reminders_treeview(self, reminders):
        """Update the Treeview with new reminders."""
