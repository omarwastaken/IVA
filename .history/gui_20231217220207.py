 ...
 # ======================== Focus Mode Feature ========================            
    ...
    def update_timer(self):
        if self.current_time > 0:
            ...
            # Create multiple arcs with varying opacities for a smoother effect
            for i in range(1, 4):
                alpha = 255 - i * 40
                color = self.timer_canvas.winfo_rgb(self.FONT_COLOR) + (alpha,)
                hex_color = "#%02x%02x%02x%02x" % color
                self.timer_canvas.create_arc(20, 20, 180, 180, start=90, extent=-extent, outline=hex_color, width=2, style=tk.ARC, tags="timer_arc")
