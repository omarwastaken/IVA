...
class ChatInterface:
    ...
    def hide_typing_animation(self):
        """Hide the typing animation from the chat interface without removing the bubble."""
    ...
    def process_and_display_response(self, message):
        ...
        # Replace the lambda function to just call add_ai_message without checking for response
        self.root.after(0, lambda: self.add_ai_message(response if response else "No response available"))
