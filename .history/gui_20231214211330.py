...
class ChatInterface:
    ...
    def setup_feature_components(self):
        ...
        # Initial display of reminders for the day
        self.display_reminders('day')

    def display_reminders(self, period):
        """Fetch and display reminders for the specified period ('day' or 'week') in the Todo list box."""
        try:
            reminders = get_reminders_for_period(period)
            print("Debug - Reminders fetched:", reminders)  # Debug: print fetched reminders

            self.todo_list_box.delete(0, tk.END)  # Clear existing items
            if reminders:
                for reminder in reminders:
                    # Format the display of each reminder
                    display_text = f"{reminder['summary']} - {reminder['start'].get('dateTime', 'No start time')}"
                    self.todo_list_box.insert(tk.END, display_text)
            else:
                self.todo_list_box.insert(tk.END, "No reminders found.")
        except Exception as e:
            self.todo_list_box.insert(tk.END, f"Error fetching reminders: {e}")
            print("Debug - Error fetching reminders:", e)   
