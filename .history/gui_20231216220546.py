...
class ChatInterface:
    ...
    def setup_reminders_components(self):
        ...
        self.todo_list_tree.column('Date', width=150, anchor='center')
        ...
        try:
            ...
            if reminders:
                for reminder in reminders:
                    ...
                    if 'dateTime' in reminder.get('start', {}):
                        # Specific time event with more emphasis on time
                        start_datetime_str = reminder['start']['dateTime']
                        end_datetime_str = reminder.get('end', {}).get('dateTime', start_datetime_str)
                        start_datetime_obj = datetime.datetime.fromisoformat(start_datetime_str)
                        end_datetime_obj = datetime.datetime.fromisoformat(end_datetime_str)
                        local_timezone = pytz.timezone(reminder['start'].get('timeZone', 'UTC'))
                        local_start_datetime = start_datetime_obj.astimezone(local_timezone)
                        local_end_datetime = end_datetime_obj.astimezone(local_timezone)
                        formatted_datetime = "{} to {} ({})".format(
                            local_start_datetime.strftime('%H:%M'),
                            local_end_datetime.strftime('%H:%M'),
                            local_start_datetime.strftime('%A, %d %B %Y')
                        )
                    elif 'date' in reminder.get('start', {}):
                        # All-day event
                        date_str = reminder['start']['date']
                        date_obj = datetime.datetime.fromisoformat(date_str)
                        formatted_datetime = date_obj.strftime('All-day (%A, %d %B %Y)')
                    else:
                        # No date information
                        formatted_datetime = "No Date"

                    self.todo_list_tree.insert('', tk.END, text='✓', values=(summary, formatted_datetime), iid=reminder.get('id', ''))
            else:
