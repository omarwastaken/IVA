...
from modules.utils import recognize_speech, speak, INPUT_MODE
...
class ChatInterface:
    ...
    """Toggle the input state between typing and speech-to-text."""
    global INPUT_MODE
    if INPUT_MODE == 1:  # Switch to speech-to-text
        INPUT_MODE = 2
        self.text_field['state'] = tk.DISABLED
        self.send_button['state'] = tk.DISABLED
        speak("Switched to voice mode. Please speak your message.")
        self.root.after(500, self.voice_input)  # Short delay before capturing voice
    else:  # Switch back to typing
        INPUT_MODE = 1
        self.text_field['state'] = tk.NORMAL
        self.send_button['state'] = tk.NORMAL
        speak("Switched back to typing mode.")
