...
class ChatInterface:
    ...
    def __init__(self, root):
        ...
        self.next_bubble_y = self.START_MARGIN
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        if not self.is_typing_animation_active:
            self.next_bubble_y += label_height + self.BUBBLE_SPACING
    ...
    if self.typing_bubble_id is not None:
        if self.typing_bubble_id < len(self.chat_bubbles):
            ...
            self.chat_bubbles.pop(self.typing_bubble_id)
            # Since we're removing the typing bubble, decrease the y position
            # for the next bubble, if the typing bubble was the last bubble added
            if self.typing_bubble_id == len(self.chat_bubbles):
                self.next_bubble_y -= (self.BUBBLE_SPACING + self.PADDING * 2)
        self.typing_bubble_id = None
    self.is_typing_animation_active = False
    self.refresh_chat_view()
    ...
    def refresh_chat_view(self):
        ...
        # Only scroll to the end if the user is not currently scrolled up to read previous messages
        if self.chat_canvas.yview()[1] == 1.0:  # Check if the scrollbar is at the bottom
