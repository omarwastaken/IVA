...
def handle_reminder_confirmation(user_input, context_data):
    """
    Handle the confirmation response for a reminder.
    """
    if any(phrase in user_input.lower() for phrase in CONFIRMATION_PHRASES):
        save_to_file("reminder", context_data["current_reminder"])
        context_data["awaiting_reminder_confirmation"] = False
        return "Reminder saved successfully!"
    elif any(phrase in user_input.lower() for phrase in DECLINING_PHRASES):
        context_data["awaiting_reminder_confirmation"] = False
        return "Reminder not saved."
    else:
        return "I didn't understand your response. Please confirm again."
