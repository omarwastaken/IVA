...
class ChatInterface:
    ...
    def calculate_bubble_position(self, label_width, is_user):
        if is_user:
            # User message bubble position calculations
            ...
            window_x = self.CHAT_AREA_WIDTH - label_width - self.WINDOW_OFFSET - self.PADDING
        else:
            # AI message bubble position calculations
            bubble_x1 = self.LABEL_OFFSET + self.PADDING
            bubble_x2 = label_width + self.LABEL_OFFSET + self.PADDING
            window_x = self.WINDOW_OFFSET + self.LABEL_OFFSET + self.PADDING
    ...
    def create_message_label(self, message_label, y_position, label_height, window_x):
        # Ensure the alignment of text within the bubble is consistent
        anchor = "e" if window_x > self.CHAT_AREA_WIDTH / 2 else "w"
        message_label_window = self.chat_canvas.create_window(
            window_x, y_position + label_height / 2, 
            window=message_label, anchor=anchor
        )
    ...
    def add_ai_message(self, message):
        ...
        if self.is_typing_animation_active and self.typing_bubble_id is not None:
            ...
            # Ensure the AI message bubbles are positioned with the correct offsets
