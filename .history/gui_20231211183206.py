...
class ChatInterface:
    ...
    def refresh_chat_view(self):
        ...
        _, yview = self.chat_canvas.yview()
        if yview == 1:
    ...
    def send_message(self, event=None):
        ...
        if message:
            # Determine if we should scroll down after adding the message
            _, yview = self.chat_canvas.yview()
            scroll_down = yview == 1
            # Add the message bubble
            self.add_message_bubble(message, is_user=True)
            # Refresh the chat view and scroll down if necessary
            self.refresh_chat_view(scroll_down)
