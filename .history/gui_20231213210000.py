...
class ChatInterface:
    ...
    def setup_ui(self):
        ...
        self.chat_text = tk.Text(self.top_frame, bg="white", fg="black", font=font.Font(family="Helvetica", size=12), state=tk.DISABLED, wrap=tk.WORD)
    ...
    def process_and_display_response(self, message):
        ...
        self.root.after(0, lambda: self.append_message("--> IVA: " + response) if response else None)
        speak(response)
    ...
    def stop_typing_animation(self):
        ...
        if hasattr(self, 'typing_animation_id'):
            ...
            self.replace_last_message("")  # Clear the last message (typing animation)

    def replace_last_message(self, new_message):
        """Replace the last message in the chat text widget."""
        self.chat_text.config(state=tk.NORMAL)
        self.chat_text.delete("end-2l", "end-1l")  # Delete the last line
        if new_message:
