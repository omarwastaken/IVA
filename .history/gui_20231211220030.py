...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        # Calculate label dimensions and bubble positions
        ...
        # Adjust the y_position based on the number of bubbles and ensure there is a top margin for the first bubble
        y_position = self.START_MARGIN if not self.chat_bubbles else self.chat_bubbles[-1][1] + self.BUBBLE_SPACING
        
        # Create and add the bubble to the canvas
        ...
        self.chat_bubbles.append((bubble, message_label_window, y_position + label_height))  # Store the bottom y-position for the next bubble
        
        # Refresh the chat view to update positions
    ...
    def calculate_bubble_position(self, label_width, is_user):
        """Calculate bubble position on the canvas."""
        # For user, position bubbles on the right; for AI, on the left
        if is_user:
            bubble_x1 = self.CHAT_AREA_WIDTH - label_width - self.LABEL_OFFSET - self.PADDING * 2
            ...
            bubble_x2 = label_width + self.LABEL_OFFSET + self.PADDING * 2
            window_x = self.LABEL_OFFSET + self.WINDOW_OFFSET
