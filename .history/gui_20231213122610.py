...
import threading
from main import process_input, handle_input
...
class ChatInterface:
    ...
    def send_message(self, event=None):
        """Handle sending of a text message."""
        ...
        if message:
            ...
            threading.Thread(target=self.process_and_display_response, args=(message, False), daemon=True).start()

    def process_and_display_response(self, message, is_voice_input):
        """Process the user's message and display the AI's response."""
        self.root.after(0, self.show_typing_animation)
        response = handle_input(message, is_voice_input)
    ...
    def process_voice_input(self):
        ...
        self.process_and_display_response("", True)    
    ...
    def toggle_input_state(self):
        """Toggle the input state between typing and voice."""
        global INPUT_MODE
        if INPUT_MODE == 1:
            INPUT_MODE = 2
