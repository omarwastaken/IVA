...
from tkinter import font, ttk, StringVar, messagebox, PhotoImage
...
class ChatInterface:
    ...
    # Theme Constants
    BACKGROUND_COLOR = '#313c49'
    FONT_COLOR = '#e7e6e6'
    SEND_BUTTON_COLOR = '#4169E1'
    PLACEHOLDER_COLOR = '#d0d0d0'
    TODO_LIST_LABEL_BG = '#313c49'
    ...
    def apply_gradient(self, frame):
        ...
        def apply():
            ...
            if width > 1 and height > 1:  # Check if the frame has a valid size
                ...
                label.lower()  # Place the label behind other widgets
 ...
 # ======================== Chat Feature ========================
    ...
    def setup_send_button(self):
        ...
        self.send_button.place(x=250, y=10)
...
class RoundedButton:
    def __init__(self, parent, width, height, corner_radius, color, text, command=None):
        ...
        self.text = self.canvas.create_text(
            width / 2, height / 2, text=text, fill='white', font=('Arial', 12)
        )
        self.canvas.pack()
    ...
    def _on_press(self, event):
        self.canvas.itemconfig(self.shape, fill='#A3B1C6')  # Change color on click

    def _on_release(self, event):
        self.canvas.itemconfig(self.shape, fill='#4169E1')  # Change color back
        if self.command:
            self.command()
...
if __name__ == "__main__":
    try:
        root = tk.Tk()
        root.title("IVA - Harnessing Focus, Boosting Productivity – Your ADHD Ally for a Balanced Life.")
        icon = PhotoImage(file='assets\iva_icon.png')
