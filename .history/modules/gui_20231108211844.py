...
class ChatBubble(QTextEdit):
    def __init__(self, text, sender):
        ...
        if sender == "ai":
            self.setStyleSheet("QTextEdit { background-color: #DCF8C6; border-radius: 10px; }")
            self.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        else:  # user
            self.setStyleSheet("QTextEdit { background-color: #34B7F1; color: white; border-radius: 10px; }")
            self.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        
        # Enable word wrap and disable scrollbars
        self.setWordWrapMode(Qt.WordWrap)
