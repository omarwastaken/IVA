...
def get_reminders_for_period(period):
    ...
    print("Debug - Raw API Response:", events_result)  # Debug: print raw response
    events = events_result.get('items', [])
    for event in events:
        print("Debug - Event fetched:", event)
    return events

def mark_reminder_as_done(event_id):
    """Marks the reminder with the given event_id as done in Google Calendar."""
    try:
        service = get_calendar_service()
        # Update the event status to 'cancelled' or a custom method to mark as done
        service.events().update(calendarId='primary', eventId=event_id, body={'status': 'cancelled'}).execute()
        print(f"Event with ID {event_id} marked as done.")
    except Exception as e:
