...
def send_message():
    ...
    if message:
        # Increase the spacing between messages and add top offset
        bubble_spacing = 40  # Add more space between bubbles
        top_offset = 100  # Additional space at the top of the chat area
        padding = 10  # Padding inside the bubble

        # Adjust the y-position for each new message, accounting for spacing and top offset
        y_position = top_offset + (bubble_spacing + padding + padding) * len(chat_bubbles)
        ...
        # Explicitly update the scroll region to accommodate the new chat bubble
