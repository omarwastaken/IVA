...
def handle_reminder_request(user_input, restart_callback):
    ...
    if is_reminder_request(user_input):
        if any(pattern in user_input.lower() for pattern in INQUIRY_PHRASES):
            return "Yes, you can set a reminder. Please provide the details of what you'd like to be reminded about and when."
        else:
            response = create_reminder(user_input, restart_callback)
            return response
    return ""
...
def confirm_and_save_reminder(reminder_content, date_str, time_str, is_recurring):
    ...
    return reminder + "\nDo you want to save this reminder?"
...
def create_reminder(user_input, restart_callback):
    ...
    if reminder_content and time_str:
        try:
            response = confirm_and_save_reminder(reminder_content, date_str, time_str, is_recurring)
            return response
        except ValueError:
            return "Sorry, I didn't understand the time or date you said. Please say it again in a recognized format."
