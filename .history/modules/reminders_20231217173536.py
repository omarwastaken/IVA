...
def handle_reminder_request(user_input):
    ...
    if is_reminder_request(user_input):
        reminder_state.is_active = True
        return ["Please provide the details of the reminder (date, time, content)."]
    return []

def process_reminder_input(user_input):
    """Processes user inputs during the reminder creation process."""
    global reminder_state
    response_messages = []

    if not reminder_state.is_active:
        return response_messages

    if not reminder_state.date_str or not reminder_state.time_str:
        reminder_state.date_str, reminder_state.time_str, reminder_state.is_recurring = extract_datetime(user_input)
        response_messages.append("Got it. What do you want to be reminded about?")
    elif not reminder_state.reminder_content:
        reminder_state.reminder_content = extract_reminder_content(user_input, reminder_state.date_str, reminder_state.time_str)
        response_messages.extend(confirm_and_save_reminder(
            reminder_state.reminder_content,
            reminder_state.date_str,
            reminder_state.time_str,
            reminder_state.is_recurring
        ))
        reminder_state.reset()
    else:
        response_messages.append("I'm not sure what you're asking. Can you please provide the reminder details again?")
    
    return response_messages

def confirm_and_save_reminder(reminder_content, date_str, time_str, is_recurring):
    """Prepares a reminder for confirmation."""
    response_messages = []
    reminder_datetime_str = f"{date_str} {time_str}"
    ...
    response_messages.append(reminder)
    response_messages.append("Do you want to save this reminder?")

    # Save the reminder details in the state for later use
    reminder_state.reminder_details = {
...
def create_reminder(user_input, restart_callback):
    ...
    if reminder_content and time_str:
        try:
            response_from_confirm = confirm_and_save_reminder(reminder_content, date_str, time_str, is_recurring)
            response_messages.extend(response_from_confirm)
        except ValueError:
            response_messages.append("Sorry, I didn't understand the time or date you said. Please say it again in a recognized format.")
...
def get_reminder_content(user_input, date_str, time_str):
    ...
    if not reminder_content:
        sreturn("What do you want me to remind you about?")
