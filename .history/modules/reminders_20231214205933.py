...
def get_reminders_for_period(period):
    """Fetches reminders for the specified period ('day' or 'week')."""
    service = get_calendar_service()
    ...
    if period == 'day':
        ...
        time_max = (now + datetime.timedelta(weeks=1)).isoformat() + 'Z'
    else:
    ...
    print("Raw API Response:", events_result)  # Debug: print raw response

    events = events_result.get('items', [])

    reminders = [event for event in events if 'reminder' in event.get('description', '')]

    print("Filtered Reminders:", reminders)  # Debug: print filtered reminders
