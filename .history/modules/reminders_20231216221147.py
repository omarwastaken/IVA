...
def get_reminders_for_period(period):
    ...
    for event in events:
        ...
        if 'dateTime' in event['start']:
            ...
            formatted_time = start_time.strftime('%H:%M%p').lower()  # Format time as '11:45pm'
        elif 'date' in event['start']:
            formatted_time = "All Day"
        else:
            formatted_time = "No Time"

        formatted_events.append({'summary': event.get('summary', 'No Title'), 'time': formatted_time, 'id': event.get('id')})
