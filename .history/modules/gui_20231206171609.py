...
def send_message():
    ...
    if message:
        # Increase the spacing between messages
        bubble_spacing = 40  # Add more space between bubbles
        padding = 10  # Padding inside the bubble

        # Additional fixed offset to start the first bubble lower
        initial_offset = 200

        # Adjust the y-position for each new message, accounting for spacing
        ...
        message_label = tk.Label(chat_canvas, text=message, bg="#ADD8E6", fg="black", font=customFont, wraplength=text_width - 2*padding, justify='left')
        ...
        # Create a rounded rectangle for the chat bubble with more padding
        bubble = create_rounded_rectangle(chat_canvas,
                                          chat_canvas.winfo_width() - label_width - 20, y_position + initial_offset,
                                          chat_canvas.winfo_width() - 20, y_position + label_height + padding + initial_offset,
                                          radius=20,
                                          fill="#ADD8E6")
        ...
        chat_canvas.update_idletasks()  # Ensure the canvas updates before moving the view
