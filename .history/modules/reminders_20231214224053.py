...
def mark_reminder_as_done(event_id):
    """Marks a reminder as done in Google Calendar."""
    service = get_calendar_service()
    try:
        ...
        if 'recurringEventId' not in event:
            # Non-recurring event: delete or update the event
            # service.events().delete(calendarId='primary', eventId=event_id).execute()  # To delete
            event['status'] = 'cancelled'
            service.events().update(calendarId='primary', eventId=event_id, body=event).execute()
        else:
            # Recurring event: handle according to your application logic
            print("Recurring event handling not implemented.")
