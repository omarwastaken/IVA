...
# Bind the canvas width to a variable so we can use it for bubble placement
def on_resize(event):
    # Update the canvas's width in the variable
    chat_canvas.width = event.width

chat_canvas.bind('<Configure>', on_resize)
chat_canvas.width = chat_canvas.winfo_reqwidth()
