...
from modules.utils import speak, save_to_file
...
def handle_reminder_request(user_input, restart_callback, gui_callback):
    """Handles a reminder request from user input."""
    if not is_reminder_request(user_input):
        return False
    if any(pattern in user_input.lower() for pattern in INQUIRY_PHRASES):
        gui_callback("Yes, you can set a reminder. Please provide the details.")
    else:
        create_reminder(user_input, restart_callback, gui_callback)
    return False
