...
class ChatInterface:
    ...
    def __init__(self, root):
        ...
        self.last_bubble_y_position = self.START_MARGIN
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        y_position = self.calculate_y_position(is_user)
        ...
        bubble_id = self.create_and_add_bubble(bubble_x1, y_position, bubble_x2, label_height, message, window_x)
        self.update_scroll_region()
        return bubble_id

    def calculate_y_position(self, is_user):
        """Calculate the Y position for a new message bubble."""
        if not self.chat_bubbles and not is_user:
            return self.START_MARGIN
        return self.last_bubble_y_position + self.BUBBLE_SPACING + self.PADDING * 2
    ...
    def create_and_add_bubble(self, bubble_x1, y_position, bubble_x2, label_height, message, window_x):
        """Create and add a new bubble on the canvas."""
        ...
        # Append the bubble and label window to the list of chat bubbles
        self.chat_bubbles.append((bubble, message_label_window))
        # Update the last bubble y position for the next message
        self.last_bubble_y_position = y_position + label_height
    ...
    def hide_typing_animation(self):
        ...
        if self.typing_bubble_id is not None:
            if self.typing_bubble_id < len(self.chat_bubbles):
                ...
                self.last_bubble_y_position -= (self.BUBBLE_SPACING + self.PADDING * 2)
            self.typing_bubble_id = None
        self.is_typing_animation_active = False
    ...
    def refresh_chat_view(self):
        ...
        if self.chat_bubbles:
            _, last_message_label_window = self.chat_bubbles[-1]
            self.chat_canvas.yview_moveto(self.chat_canvas.bbox(last_message_label_window)[1] / self.chat_canvas.bbox("all")[3]) 
