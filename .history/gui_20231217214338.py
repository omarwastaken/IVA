...
class ChatInterface:
 ...
 # ======================== Focus Mode Feature ========================            
    ...
    def setup_focus_mode_components(self):
        ...
        self.circle = self.timer_canvas.create_oval(20, 20, 180, 180, outline=self.FONT_COLOR, width=2)
        ...
        self.minus_five_button = tk.Button(self.focus_mode_frame, text="-5", command=self.decrease_timer, bg=self.SEND_BUTTON_COLOR, fg="white")
        self.minus_five_button.pack(side=tk.LEFT, padx=20, pady=10)
        self.plus_five_button = tk.Button(self.focus_mode_frame, text="+5", command=self.increase_timer, bg=self.SEND_BUTTON_COLOR, fg="white")
        self.plus_five_button.pack(side=tk.LEFT, padx=20, pady=10)
    ...
    def update_timer(self):
        if self.current_time > 0:
            ...
            extent = (elapsed_time / self.POMODORO_TIME) * 360

            # Update the circle timer
            self.timer_canvas.delete("timer_arc")  # Delete the old arc
            self.timer_canvas.create_arc(20, 20, 180, 180, start=90, extent=-extent, outline=self.FONT_COLOR, width=2, style=tk.ARC, tags="timer_arc")
 ...
 # ======================== Focus Mode Feature Event Handling ========================            
    ...
    def reset_focus_timer(self):
        ...
        self.timer_canvas.itemconfig(self.timer_text, text="25:00")
        self.timer_canvas.delete("timer_arc")  # Delete the old arc
        self.timer_canvas.create_oval(20, 20, 180, 180, outline=self.FONT_COLOR, width=2)

    def increase_timer(self):
        if not self.is_timer_running:
            self.current_time = min(self.current_time + 5 * 60, self.POMODORO_TIME + 30 * 60)  # Limit to max 55 minutes
            self.update_timer_display()

    def decrease_timer(self):
        if not self.is_timer_running:
            self.current_time = max(self.current_time - 5 * 60, 5 * 60)  # Limit to min 5 minutes
            self.update_timer_display()
