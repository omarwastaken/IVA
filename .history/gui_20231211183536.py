...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        y_position = self.next_bubble_y  # Use the new variable for y position
        ...
        if not self.is_typing_animation_active:  # Only update next_bubble_y if it's not a typing animation
            self.next_bubble_y += label_height + self.BUBBLE_SPACING  # Update the y position for the next bubble

        self.refresh_chat_view()  # Ensure the view is updated to include the new bubble
        return bubble_id
