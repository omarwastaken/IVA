...
class ChatInterface:
    ...
    def toggle_input_state(self):
        ...
        self.mic_mode = not self.mic_mode  # Toggle the microphone mode
        if self.mic_mode:
            self.text_field['state'] = tk.DISABLED
            self.send_button['state'] = tk.DISABLED
            self.listen_and_convert_speech_continuously()
        else:
            self.text_field['state'] = tk.NORMAL
            self.send_button['state'] = tk.NORMAL

    def listen_and_convert_speech_continuously(self):
        """Continuously listen to speech and convert it to text until mic mode is off."""
        while self.mic_mode:
            text = recognize_speech()  # From utils.py
            if text:
                self.root.after(0, lambda: self.process_text_input(text))
            else:
                break  # Exit the loop if no text is recognized
