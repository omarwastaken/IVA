...
class ChatInterface:
    ...
    def toggle_input_state(self):
        ...
        global INPUT_MODE
        if INPUT_MODE == 1:
            INPUT_MODE = 2  # Switch to voice input
            self.text_field['state'] = tk.DISABLED
            self.send_button['state'] = tk.DISABLED
            # Change mic_button appearance to indicate active listening
        else:
            INPUT_MODE = 1  # Switch to text input
            self.text_field['state'] = tk.NORMAL
            self.send_button['state'] = tk.NORMAL
            # Revert mic_button appearance

def handle_voice_input(self):
    """Process voice input."""
    voice_input = recognize_speech()
    if voice_input:
        self.process_and_display_response(voice_input)               
