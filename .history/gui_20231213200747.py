...
class ChatInterface:
    ...
    def process_and_display_response(self, message):
        self.manage_typing_animation("show")  # Start the typing animation
        response = process_input(message, self.previous_questions_and_answers)
        self.root.after(0, lambda: [self.manage_typing_animation("hide"), self.add_ai_message(response) if response else None])
        speak(response)
        if self.mic_mode:
    ...
    def add_ai_message(self, message):
        ...
        if self.typing_bubble_id is None:
            self.typing_bubble_id = self.add_message_bubble("IVA is typing...", is_user=False)
        self.update_message_bubble(self.typing_bubble_id, message)
        self.is_typing_animation_active = False
        self.manage_typing_animation("hide")
    ...
    def update_message_bubble(self, bubble_id, new_message):
        ...
        # Check if the bubble exists in the canvas
        if bubble_id in self.chat_bubbles:
            # Update the text of the label associated with the bubble
            message_label = self.chat_bubbles[bubble_id]
            message_label.config(text=new_message)
        else:
            # If the bubble is not found, log an error or handle appropriately
            print(f"Bubble with ID {bubble_id} not found.")

    # ======================== Animation and Visual Effects ======================== 

    def manage_typing_animation(self, action):
        """Manage the typing animation in the chat interface."""
        if action == "show":
            if self.typing_animation_id is None:
                ...
                self.manage_typing_animation("animate")
            ...
            if self.is_typing_animation_active:
                ...
                self.typing_animation_id = self.root.after(500, lambda: self.manage_typing_animation("animate"))
            ...
            if self.typing_bubble_id is not None:
                self.chat_canvas.delete(self.typing_bubble_id)
                self.typing_bubble_id = None
            self.is_typing_animation_active = False
            self.refresh_chat_view()
