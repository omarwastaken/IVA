...
import threading
import keyboard
from modules.utils import speak, save_to_file, recognize_speech, is_mode_switched
...
def on_ctrl_m_press():
    global input_mode, is_mode_switched
    is_mode_switched = True
...
def talk_iva():
    ...
    while True:
        if input_mode == 'voice':
            ...
            if is_mode_switched:  # Check if mode was switched during voice recognition
                is_mode_switched = False
                continue  # Skip to next iteration for immediate switch handling
        else:
