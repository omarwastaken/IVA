...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        # Apply START_MARGIN for the first message bubble regardless of user or AI
        if not self.chat_bubbles:
            ...
            # Add START_MARGIN to the y_position if it's the AI's first message
            y_position = (last_bubble_coords[3] + self.BUBBLE_SPACING) if is_user else (self.START_MARGIN + self.BUBBLE_SPACING)
    ...
    def calculate_bubble_position(self, label_width, is_user):
        if is_user:
            ...
            bubble_x1 = self.LABEL_OFFSET + self.PADDING  # Increased left padding for AI messages
