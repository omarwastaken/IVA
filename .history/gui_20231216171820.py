...
class ChatInterface:
    ...
    def setup_feature_components(self):
        ...
        self.todo_list_label = tk.Label(self.features_frame, text="Todo List", bg='#f0f0f0', fg='#333', font=('Arial', 12, 'bold'))
        self.todo_list_label.pack(padx=20, pady=(10, 5))

        style = ttk.Style()
        style.configure("Treeview.Heading", font=('Calibri', 11, 'bold'), foreground='blue')
        ...
        try:
            ...
            if reminders:
                for reminder in reminders:
                    # Check if 'dateTime' key is in the 'start' dictionary
                    if 'dateTime' in reminder.get('start', {}):
                        ...
                        local_timezone = pytz.timezone(reminder['start'].get('timeZone', 'UTC'))  # Default to UTC if timezone is missing
                        local_datetime = datetime_obj.astimezone(local_timezone)
                        formatted_datetime = local_datetime.strftime('%H:%M, %A, %d %B %Y')
                    else:
                        # Handle reminders without a dateTime
                        formatted_datetime = "No Date"

                    self.todo_list_tree.insert('', tk.END, text='✓', values=(reminder.get('summary', 'No Title'), formatted_datetime), iid=reminder.get('id', ''))
            else:
