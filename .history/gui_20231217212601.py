 ...
 # ======================== Focus Mode Feature ========================            
    ...
    def update_timer(self):
        ...
        if self.current_time > 0:
            ...
            self.update_circular_timer()

    def create_circular_timer(self):
        self.canvas = tk.Canvas(self.focus_mode_frame, width=200, height=200, bg=self.BACKGROUND_COLOR, highlightthickness=0)
        self.canvas.pack(pady=(20, 0))
        self.circle = self.canvas.create_oval(10, 10, 190, 190, outline=self.FONT_COLOR, width=6)
        self.timer_text = self.canvas.create_text(100, 100, text=self.timer_var.get(), fill=self.FONT_COLOR, font=('Arial', 24, 'bold'))

    def update_circular_timer(self):
        self.canvas.itemconfig(self.timer_text, text=self.timer_var.get())
        angle = (self.current_time / self.POMODORO_TIME) * 360
        x, y = 100 - 90 * cos(angle * pi / 180), 100 - 90 * sin(angle * pi / 180)
        self.canvas.coords(self.circle, 100, 100, x, y)
