...
class ChatInterface:
    ...
    def add_message_bubble(self, message, is_user=True):
        ...
        return bubble_id, message_label
    ...
    def update_message_bubble(self, bubble_id, new_message):
        ...
        if bubble_id in self.chat_bubbles:
            # Directly updating the text of the label widget
            label_widget = self.chat_bubbles[bubble_id]
            label_widget.config(text=new_message)

            # Adjusting the bubble margin if needed
            self.bubble_margin_top = label_widget.winfo_y() + label_widget.winfo_height() + self.BUBBLE_SPACING
            self.refresh_chat_view()
